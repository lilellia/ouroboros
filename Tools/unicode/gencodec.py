...
def hexrepr(t, precision=4):
    ...
    try:
        ...
        return "0x%0*X" % (precision, t)  # noqa: UP031
    try:
        return "(" + ", ".join(["0x%0*X" % (precision, item) for item in t]) + ")"  # noqa: UP031
...
def python_mapdef_code(varname, map, comments=1, precisions=(2, 4)):
    ...
    append = l.append
    if "IDENTITY" in map:
        append("%s = codecs.make_identity_dict(range(%d))" % (varname, map["IDENTITY"]))  # noqa: UP031
...
def python_tabledef_code(varname, map, comments=1, key_precision=2):
    ...
    if maxchar < 256:
        append(f"    {UNI_UNDEFINED!a} \t## Widen to UCS2 for optimization")
    append(")")
    return l
...
class Codec(codecs.Codec):
    ...
    def encode(self, input, errors='strict'):
        return codecs.charmap_encode(input, errors, encoding_{suffix})

    def decode(self, input, errors='strict'):
        return codecs.charmap_decode(input, errors, decoding_{suffix})
'''
    ]
    l.append(
        f"""\
class IncrementalEncoder(codecs.IncrementalEncoder):
    def encode(self, input, final=False):
        return codecs.charmap_encode(input, self.errors, encoding_{suffix})[0]

class IncrementalDecoder(codecs.IncrementalDecoder):
    def decode(self, input, final=False):
        return codecs.charmap_decode(input, self.errors, decoding_{suffix})[0]"""
    )
