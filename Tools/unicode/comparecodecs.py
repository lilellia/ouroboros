...
def compare_codecs(encoding1, encoding2):
    ...
    print(f"Comparing encoding/decoding of   {encoding1!r} and   {encoding2!r}")
    mismatch = 0
    # Check encoding
    for i in range(sys.maxunicode + 1):
        ...
        try:
            ...
            c2 = "<undefined>"
        if c1 != c2:
            print(" * encoding mismatch for 0x%04X: %-14r != %r" % (i, c1, c2))  # noqa: UP031
            mismatch += 1
    # Check decoding
    for i in range(256):
        c = bytes([i])
        ...
        try:
            ...
            u2 = "<undefined>"
        if u1 != u2:
            print(" * decoding mismatch for 0x%04X: %-14r != %r" % (i, u1, u2))  # noqa: UP031
            mismatch += 1
    if mismatch:
        print()
        print("Found %i mismatches" % mismatch)  # noqa: UP031
    else:
