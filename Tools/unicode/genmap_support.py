    ...
    def update_decode_map(self, c1range, c2range, onlymask=(), wide=0):
        ...
        for c1 in range(c1range[0], c1range[1] + 1):
            ...
            for v in range(rc2values[0], rc2values[-1] + 1):
                if v in c2map:
                    self.filler.write("%d," % c2map[v])  # noqa: UP031
                else:
                    self.filler.write("U,")
    ...
    def generate(self, wide=False):
        ...
        for i in range(256):
            ...
            self.filler.write(
                ...
                f"__{prefix}_decmap",
                "+",
                "%d" % m[i]["midx"],  # noqa: UP031
                ",",
                "%d," % m[i]["min"],  # noqa: UP031
                "%d" % m[i]["max"],  # noqa: UP031
                "},",
            )
        self.filler.printout(self.fp)
        self.fp.write("};\n\n")
    ...
    def printmap(self):
        ...
        for i in range(256):
            if i in self.encode_map and self.prefix in self.encode_map[i]:
                self.filler.write(
                    "{",
                    f"__{self.prefix}_encmap",
                    "+",
                    "%d" % self.encode_map[i]["midx"],  # noqa: UP031
                    ",",
                    "%d," % self.encode_map[i]["min"],  # noqa: UP031
                    "%d" % self.encode_map[i]["max"],  # noqa: UP031
                    "},",
                )
            else:
                self.filler.write("{", "0,", "0,", "0", "},")
