...
def run_cmd(
    ...
    **kwargs,
    ...
    proc = subprocess.run(argv, env=env, **kwargs)  # noqa: PLW1510
    return proc.stdout
...
def preprocess(tool, filename, cwd=None, **kwargs):
    ...
    if _expected := is_os_mismatch(filename):
        error = None
        raise OSMismatchError(filename, _expected, argv, error, TOOL)  # noqa: F821
...
def is_os_mismatch(filename, errtext=None):
    ...
    if errtext is not None and (missing := is_missing_dep(errtext)):
...
def get_matching_oses(missing, filename):
    ...
    if (
        "darwin" in filename
        or "osx" in filename
        or missing == "SystemConfiguration/SystemConfiguration.h"
    ):
        return ("darwin",)
