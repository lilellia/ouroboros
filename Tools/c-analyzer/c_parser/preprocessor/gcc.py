...
def _iter_lines(text, reqfile, samefiles, cwd, raw=False):
    ...
    for line in lines:
        ...
        _lno, included, flags = _parse_marker_line(line, reqfile)
        if not included:
    ...
    for line in lines:
        ...
        if included:
            ...
            if m := PREPROC_DIRECTIVE_RE.match(line):
                (name,) = m.groups()
                if name != "pragma":
                    raise Exception(line)  # noqa: TRY002
            else:
...
def _parse_marker_line(line, reqfile=None):
    ...
    flags = {int(f) for f in flags.split()} if flags else ()
