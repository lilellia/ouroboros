...
class KIND(enum.Enum):
    ...
    def resolve_group(cls, group):
        if isinstance(group, cls):
            ...
            resolved = set()
            for gr in group:
                resolve.update(cls.resolve_group(gr))  # noqa: F821
            return resolved
...
class ParsedItem(namedtuple("ParsedItem", "file kind parent name data")):
    ...
    def from_row(cls, row, columns=None):
        if not columns:
            colnames = ["filename", "funcname", "name", "kind", "data"]
        else:
    ...
    def render_rowdata(self, columns=None):
        ...
        for column, value in rowdata.items():
            ...
            if column == "kind":
                ...
                if column == "parent" and self.parent:
...
def _fmt_line(parsed, data=None):
    ...
    parts.extend(
        [
            name,
            f"<{data}>" if data else "-",
            f"({parsed.file or '<unknown file>'!s})",
        ]
    )
    yield "\t".join(parts)
...
class Variable(Declaration):
    ...
    def _format_data(cls, fmt, data, extra):
        ...
        if fmt in ("line", "brief") or fmt == "row":
            yield text
        else:
            raise NotImplementedError(fmt)
    ...
    def __init__(self, file, name, data, parent=None, storage=None):
        ...
        if storage and storage not in STORAGE:
...
class Function(Declaration):
    ...
    def _unformat_data(cls, datastr, fmt=None):
        if fmt in ("line", "brief") or fmt == "row":
            sig, storage = Signature.from_str(sig)  # noqa: F821
...
class TypeDef(TypeDeclaration):
    ...
    def _format_data(cls, fmt, data, extra):
        text = str(data)
        if fmt in ("line", "brief") or fmt == "full" or fmt == "row":
            yield text
        else:
            raise NotImplementedError(fmt)
