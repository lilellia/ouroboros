from ..info import KIND, FileInfo, ParsedItem
...
class SourceInfo:
    ...
    def nest(self, text, before, start=None):
        if self._current is None:
            raise Exception("nesting requires active source text")  # noqa: TRY002
    ...
    def resume(self, remainder=None):
        if not self._nested:
            raise Exception("no nested text to resume")  # noqa: TRY002
        if self._current is None:
            raise Exception("un-nesting requires active source text")  # noqa: TRY002
    ...
    def advance(self, remainder, start=None):
        if self._current is None:
            raise Exception("advancing requires active source text")  # noqa: TRY002
        if remainder.strip():
