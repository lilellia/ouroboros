...
def parse_function_body(name, text, resolve, source, anon_name, parent):  # noqa: F811
    raise NotImplementedError
    ...
    while depth > 0:
        ...
        while not m:
            text, resolve = continue_text(source, text or "{", resolve)  # noqa: F821
            m = LOCAL_RE.match(text)
        text = text[m.end() :]
        (
            empty,
        ...
        if empty:
            ...
            ident = f"{kind} {name}"
            for member, inline, text in _parse_body(  # noqa: B020
            ...
            try:
                ...
                # resolve(None, None, None, text)
                text, resolve = continue_text(source, text, resolve)  # noqa: F821
                yield None, text
            else:
                head = text[:pos]
                text = text[pos:]
    ...
    while depth > 0:
        for srcinfo in source:
            ...
            if srcinfo is not None:
                srcinfo.done()
            return
        for item, depth in _parse_next_local_static(m, srcinfo, anon_name, func, depth):  # noqa: B020
            if callable(item):
                parse_body = item
