...
def write_decls(decls, outfile, fmt=None, *, backup=False):
    if fmt is None:
        fmt = _get_format(infile)  # noqa: F821
...
def _write_decls_tsv(decls, outfile, extracolumns, kwargs):
    ...
    if extracolumns:
        ...
        def render_decl(decl):
            if type(row) is tuple:  # noqa: F821
                decl, *extra = decl
            else:
                extra = ()
            extra += ("???",) * (len(extraColumns) - len(extra))  # noqa: F821
            *row, declaration = _render_known_row(decl)  # noqa: F821
...
def _render_known_decl(
    ...
    *,
    ...
    if _columns is None:
        _columns = ["filename", "parent", "name", "kind", "data"]
