...
def read_table(
    ...
    *,
    ...
    if isinstance(infile, str):
        with _open(infile, newline="") as infile:  # noqa: PLR1704
...
def write_table(
    ...
    *,
    ...
    if isinstance(outfile, str):
        with _open(outfile, "w", newline="") as outfile:  # noqa: PLR1704
            return write_table(
                outfile,
                header,
                rows,
    ...
    if (ncols or 0) > 0:
        ...
        if diff:
            if default is NOT_SET or diff < 0:
                raise Exception(f"bad row (expected {ncols} columns, got {row!r})")  # noqa: TRY002
            row += (default,) * diff
    return row
...
class ColumnSpec(namedtuple("ColumnSpec", "field label fmt")):
    ...
    def _normalize(cls, spec):
        if len(spec) == 1:
            (raw,) = spec
            raise NotImplementedError
            return _resolve_column(raw)  # noqa: F821
