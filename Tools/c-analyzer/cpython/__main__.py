...
FILES_KWARGS = {"excluded": _parser.EXCLUDED, "nargs": "*"}
        ...
        def analyze(files, **kwargs):
            ...
            for decl in _analyzer.iter_decls(files, **kwargs):
                if not KIND.is_type_decl(decl.kind):
                    continue
                if not decl.filename.endswith(".h"):  # noqa: SIM102
...
def _cli_capi(parser):
    ...
    def process_format(args, *, argv=None):
        ...
        if isinstance(args.format, str) and args.format not in _capi._FORMATS:
...
def _cli_builtin_types(parser):
    ...
    def process_format(args, *, argv=None):
        ...
        if isinstance(args.fmt, str):  # noqa: SIM102
