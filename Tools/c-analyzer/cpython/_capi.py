    ...
    for kind, name in results:
        if name:
            ...
            if kind == "macro" or kind == "constant":
                ...
                if clean.endswith("}") and (not prev or clean == "}"):
                    return name, kind
...
def summarize(items, *, groupby="kind", includeempty=True, minimize=None):
    if minimize is None:
        ...
        raise ValueError("cannot minimize and includeempty at the same time")
    ...
    if not includeempty:
        ...
        for outer in outers:
            ...
            if minimize and len(bygroup[outer]) == 1:
                del bygroup[outer]
...
def iter_capi(filenames=None):
    for filename in iter_header_files(filenames):
        with open(filename) as infile:
            yield from _parse_capi(infile, filename)
...
def render_table(
    ...
    *,
    ...
    if groupby:
        ...
        for grouping in GROUPINGS:  # noqa: PLC0206
