...
def main():
    ...
    while pos < len(sys.argv) - 1:
        ...
        if sys.argv[pos] == "-i":
            try:
                ...
                usage(
                    f"File name '{sys.argv[pos + 1]}' specified with the -i option "
                    f"can not be read - {why}"
                )
    ...
    version = "%d.%d" % sys.version_info[:2]  # noqa: UP031
    ...
    if ishome:
        ...
        incldir = os.path.join(prefix, "include", f"python{flagged_version}")
        config_h_dir = os.path.join(exec_prefix, "include", f"python{flagged_version}")
    ...
    for dir in check_dirs:
        if not os.path.exists(dir):
            usage(f"needed directory {dir} not found")
        if not os.path.isdir(dir):
            usage(f"{dir}: not a directory")
    if win:
    ...
    for file in supp_sources:
        if not os.path.exists(file):
            usage(f"needed file {file} not found")
        if not os.path.isfile(file):
            usage(f"{file}: not a plain file")
    if not win:
        for dir in extensions:
            setup = os.path.join(dir, "Setup")
            if not os.path.exists(setup):
                usage(f"needed file {setup} not found")
            if not os.path.isfile(setup):
                usage(f"{setup}: not a plain file")
    ...
    for arg in args:
        ...
        if not os.path.exists(arg):
            usage(f"argument {arg} not found")
        if not os.path.isfile(arg):
            usage(f"{arg}: not a plain file")
    ...
    if odir and not os.path.isdir(odir):
        try:
            ...
            usage(f"{odir}: mkdir failed ({msg!s})")
    base = ""
    if odir:
    ...
    if error_if_any_missing:
        ...
        if missing:
            sys.exit(f"There are some missing modules: {missing!r}")
...
def usage(msg):
    ...
    print("Error:", msg)
    print(f"Use ``{sys.argv[0]} -h'' for help")
    sys.exit(2)
