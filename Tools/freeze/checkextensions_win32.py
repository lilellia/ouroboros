...
def checkextensions(unknown, extra_inis, prefix):
    ...
    if not os.path.isfile(defaultMapName):
        sys.stderr.write(
            f"WARNING: {defaultMapName} can not be found - standard extensions may not be found\n"
        )
    else:
...
def parse_dsp(dsp):
    ...
    ret = []
    dsp_path, _dsp_name = os.path.split(dsp)
    try:
        with open(dsp, "r") as fp:
            lines = fp.readlines()
    except OSError as msg:
        sys.stderr.write(f"{dsp}: {msg}\n")
        return None
    for line in lines:
        fields = line.strip().split("=", 2)
        if fields[0] == "SOURCE":  # noqa: SIM102
...
def write_extension_table(fname, modules):
    ...
    try:
        ...
        for module in modules:
            ...
            fp.write(f"extern void init{name}(void);\n")
        # Write the table
        fp.write(ext_tab_header)
        for module in modules:
            name = module.name.split(".")[-1]
            fp.write(f'\t{{"{name}", init{name}}},\n')
