...
def pprint(data):
    ...
    for k, v in items:
        print("    %-40s%a," % (f"{k!a}:", v))  # noqa: UP031
...
def check(data):
    ...
    for k, v in data.items():
        if locale.normalize(k) != v:
            print(f"ERROR: {k!a} -> {locale.normalize(k)!a} != {v!a}", file=sys.stderr)
            errors += 1
    return errors
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=f"location of the X11 alias file (default: {LOCALE_ALIAS!a})",
    )
    parser.add_argument(
        "--glibc-supported",
        default=SUPPORTED,
        help=f"location of the glibc SUPPORTED locales file (default: {SUPPORTED!a})",
    )
