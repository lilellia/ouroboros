...
# @generated by pegen from metagrammar.gram  # noqa: EXE001
...
class GeneratedParser(Parser):
    ...
    def rule(self) -> Rule | None:
        ...
        if (
            (rulename := self.rulename())
            and (opt := self.memoflag(),)
            and (literal := self.expect(":"))  # noqa: F841
            and (alts := self.alts())
            and (_newline := self.expect("NEWLINE"))
        ):
    ...
    @memoize
    def rulename(self) -> RuleName | None:
    ...
    def memoflag(self) -> str | None:
        ...
        if (
            (literal := self.expect("("))  # noqa: F841
            and (literal_1 := self.expect("memo"))  # noqa: F841
            and (literal_2 := self.expect(")"))  # noqa: F841
        ):
            return "memo"
        self._reset(mark)
        return None
    ...
    @memoize
    def forced_atom(self) -> Forced | None:
        # forced_atom: '&' '&' ~ atom
        mark = self._mark()
        cut = False
        if (
            (literal := self.expect("&"))  # noqa: F841
            and (literal_1 := self.expect("&"))  # noqa: F841
            and (cut := True)
            and (atom := self.atom())
        ):
            return Forced(atom)
    ...
    def lookahead(self) -> LookaheadOrCut | None:
        ...
        if cut:
            return None
        if literal := self.expect("~"):  # noqa: F841
            return Cut()
        self._reset(mark)
        return None
