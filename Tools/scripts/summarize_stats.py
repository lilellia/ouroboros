...
def join_rows(a_rows, b_rows):
    ...
    if len(a_rows):
        a_ncols = list({len(x) for x in a_rows})
        if len(a_ncols) != 1:
            raise ValueError("Table a is ragged")

    if len(b_rows):
        b_ncols = list({len(x) for x in b_rows})
        if len(b_ncols) != 1:
    ...
    # in A
    keys = list(a_data.keys()) + [k for k in b_data if k not in a_data]
...
def output_stats(inputs, json_output=None):
    ...
    print("---")
    print("Stats gathered on:", date.today())  # noqa: DTZ011
