...
try:
    ...
    def os_fsencode(filename):
        if not isinstance(filename, unicode):  # noqa: F821
            return filename
...
class PyObjectPtr:
    ...
    def from_pyobject_ptr(cls, gdbval):
        ...
        try:
            ...
            cls = cls.subclass_from_type(p.type())  # noqa: PLW0642
...
class InstanceProxy:
    ...
    def __repr__(self):
        if isinstance(self.attrdict, dict):
            kwargs = ", ".join([f"{arg}={val!r}" for arg, val in self.attrdict.items()])
...
class PyFramePtr:
    ...
    def write_repr(self, out, visited):
        ...
        out.write(
            f"Frame 0x{self.as_address():x}, for file {self.co_filename.proxyval(visited)}, line {lineno}, in {self.co_name.proxyval(visited)} ("
        )
        first = True
        for pyop_name, pyop_value in self.iter_locals():
            if not first:
    ...
    def print_traceback(self):
        ...
        lineno = str(lineno) if lineno is not None else "?"
        sys.stdout.write(
            f'  File "{self.co_filename.proxyval(visited)}", line {lineno}, in {self.co_name.proxyval(visited)}\n'
        )
        ...
        if self.is_evalframe():
            ...
            while True:
                if interp_frame:
                    ...
                    sys.stdout.write("#%i %s\n" % (self.get_index(), line))  # noqa: UP031
            ...
            if info:
                sys.stdout.write("#%i %s\n" % (self.get_index(), info))  # noqa: UP031
            else:
                sys.stdout.write("#%i\n" % self.get_index())  # noqa: UP031
...
class PyList(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        try:
            f = open(os_fsencode(filename), "r", encoding="utf-8")  # noqa: SIM115
        ...
        with f:
            ...
            for i, line in enumerate(all_lines[start - 1 : end]):
                ...
                if i + start == lineno:
                    linestr = ">" + linestr
                sys.stdout.write("%4s    %s" % (linestr, line))  # noqa: UP031
...
class PyLocals(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        while True:
            ...
            sys.stdout.write(f"Locals for {pyop_frame.co_name.proxyval(set())}\n")
