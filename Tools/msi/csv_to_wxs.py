...
def main(file_source, install_target):
    ...
    assert len(files) == len({make_id(f[1]) for f in files}), "Duplicate file IDs exist"
    ...
    for group in sorted(groups):
        lines.extend(
            [
                "    <Fragment>",
                f'        <ComponentGroup Id="{group}">',
            ]
        )
        for source, target, disk_id, condition in groups[group]:
            lines.append(
                f'            <Component Id="{make_id(target)}" Directory="{make_id(target.parent)}" Guid="*">'
            )
            if condition:
            ...
            if disk_id:
                lines.append(
                    f'                <File Id="{make_id(target)}" Name="{target.name}" Source="{source}" DiskId="{disk_id}" />'
                )
            else:
                lines.append(
                    f'                <File Id="{make_id(target)}" Name="{target.name}" Source="{source}" />'
                )
        ...
        if create_folders or remove_folders:
            ...
            lines.extend(
                f'                <RemoveFolder Id="Remove_{p}_folder" On="uninstall" Directory="{p}" />'
    ...
    with open(install_target, "w") as f:
        f.writelines(line + "\n" for line in lines)
    print(f"Wrote {len(lines)} lines to {install_target}")
