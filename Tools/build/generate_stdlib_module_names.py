...
def list_frozen(names):
    ...
    if submodules:
        raise Exception(f"unexpected frozen submodules: {sorted(submodules)}")  # noqa: TRY002
...
def list_modules():
    ...
    for name in names:
        if "." in name:
            raise Exception("sub-modules must not be listed")  # noqa: TRY002

    return names


def write_modules(fp, names):
    print(f"// Auto-generated by {SCRIPT_NAME}.", file=fp)
