...
def reflow_lines(s, depth):
    ...
    while len(cur) > size:
        ...
        if i == -1 and "GeneratorExp" in cur:
            i = size + 3
        assert i != -1, "Impossible line %d to reflow: %r" % (size, s)  # noqa: UP031
...
class TypeDefVisitor(EmitVisitor):
    ...
    def simple_sum(self, sum, name, depth):
        ...
        for i in range(len(sum.types)):
            type = sum.types[i]
            enum.append("%s=%d" % (type.name, i + 1))  # noqa: UP031
        enums = ", ".join(enum)
        ctype = get_c_type(name)
        s = f"typedef enum _{name} {{ {enums} }} {ctype};"
        self.emit(s, depth)
        self.emit("", depth)

    def sum_with_constructors(self, sum, name, depth):
        ctype = get_c_type(name)
        s = "typedef struct _{name} *{ctype};".format(**locals())
        self.emit(s, depth)
        self.emit("", depth)
...
typedef struct {{
    _ASDL_SEQ_HEAD
    {ctype} typed_elements[1];
}} asdl_{name}_seq;""".format(**locals()),
            reflow=False,
            depth=depth,
        )
        self.emit("", depth)
        self.emit(
            "asdl_{name}_seq *_Py_asdl_{name}_seq_new(Py_ssize_t size, PyArena *arena);".format(
                **locals()
            ),
            depth,
        )
...
class StructVisitor(EmitVisitor):
    ...
    def sum_with_constructors(self, sum, name, depth):
        ...
        for i in range(len(sum.types)):
            type = sum.types[i]
            enum.append("%s_kind=%d" % (type.name, i + 1))  # noqa: UP031
    ...
    def visitField(self, field, depth):
        ...
        if field.seq:
            if field.type in self.metadata.simple_sums:
                self.emit("asdl_int_seq *{name};".format(**locals()), depth)
            else:
                _type = field.type
                self.emit("asdl_{_type}_seq *{name};".format(**locals()), depth)
        else:
            self.emit("{ctype} {name};".format(**locals()), depth)

    def visitProduct(self, product, name, depth):
        self.emit("struct _{name} {{".format(**locals()), depth)
        ...
        for field in product.attributes:
            ...
            type = str(field.type)
            assert type in asdl.builtin_types, type
            self.emit(f"{type} {field.name};", depth + 1)
        self.emit("};", depth)
        self.emit("", depth)
...
class PrototypeVisitor(EmitVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        ...
        if args:
            ...
            argstr = "PyArena *arena"
        self.emit(f"{ctype} {ast_func_name(name)}({argstr});", False)
...
class Obj2ModVisitor(PickleVisitor):
    attribute_special_defaults = {  # noqa: RUF012
        "end_lineno": "lineno",
    ...
    def recursive_call(self, node, level):
        self.emit(
            f"if (_Py_EnterRecursiveCall(\" while traversing '{node}' node\")) {{",
            level,
            reflow=False,
        )
    ...
    def funcHeader(self, name):
        ...
        self.emit("int", 0)
        self.emit(
            f"obj2ast_{name}(struct ast_state *state, PyObject* obj, {ctype}* out, PyArena* arena)",
            0,
        )
        self.emit("{", 0)
    ...
    def sumTrailer(self, name, add_label=False):
        ...
        error = f"expected some sort of {name}, but got %R"
    ...
    def complexSum(self, sum, name):
        ...
        for t in sum.types:
            self.emit(f"tp = state->{t.name}_type;", 1)
            ...
            self.emit(f"*out = {ast_func_name(t.name)}({self.buildArgs(args)});", 2)
    ...
    def visitProduct(self, prod, name):
        ...
        self.emit(f"*out = {ast_func_name(name)}({self.buildArgs(args)});", 1)
    ...
    def visitField(self, field, name, sum=None, prod=None, depth=0):
        ...
        if field.seq:
            ...
            if not field.opt:
                self.emit("if (tmp == NULL) {", depth)
                message = f'required field \\"{field.name}\\" missing from {name}'
                ...
                if self.isNumeric(field):
                    if field.name in self.attribute_special_defaults:
                        self.emit(
                            f"{field.name} = {self.attribute_special_defaults[field.name]};",
                            depth + 1,
                        )
                    else:
        ...
        if field.seq:
            ...
            self.emit(
                f'PyErr_Format(PyExc_TypeError, "{name} field \\"{field.name}\\" must '
            ...
            self.emit(
                f'PyErr_SetString(PyExc_RuntimeError, "{name} field \\"{field.name}\\" '
            ...
            self.emit("}", depth + 2)
            self.emit(f"asdl_seq_SET({field.name}, i, val);", depth + 2)
            self.emit("}", depth + 1)
        else:
            with self.recursive_call(name, depth + 1):
                self.emit(
                    f"res = obj2ast_{field.type}(state, tmp, &{field.name}, arena);",
                    depth + 1,
                )
...
class PyTypesDeclareVisitor(PickleVisitor):
    def visitProduct(self, prod, name):
        self.emit(
            f"static PyObject* ast2obj_{name}(struct ast_state *state, struct validator *vstate, void*);",
            0,
        )
        if prod.attributes:
            self.emit(f"static const char * const {name}_attributes[] = {{", 0)
            for a in prod.attributes:
                self.emit(f'"{a.name}",', 1)
            self.emit("};", 0)
        if prod.fields:
            self.emit(f"static const char * const {name}_fields[]={{", 0)
            for f in prod.fields:
    ...
    def visitSum(self, sum, name):
        ...
        self.emit(
            f"static PyObject* ast2obj_{name}(struct ast_state *state, struct validator *vstate, {ptype});",
            0,
        )
        for t in sum.types:
    ...
    def visitConstructor(self, cons, name):
        if cons.fields:
            self.emit(f"static const char * const {cons.name}_fields[]={{", 0)
...
""",
    ...
    def visitProduct(self, prod, name):
        if prod.fields:
            ...
            fields = "NULL"
        self.emit(
            'state->%s_type = make_type(state, "%s", state->AST_type, %s, %d,'  # noqa: UP031
            % (name, name, fields, len(prod.fields)),
            1,
        )
        self.emit(f"{reflow_c_string(asdl_of(name, prod), 2)});", 2, reflow=False)
        self.emit(f"if (!state->{name}_type) return 0;", 1)
        if prod.attributes:
            self.emit(
                "if (!add_attributes(state, state->%s_type, %s_attributes, %d)) return 0;"  # noqa: UP031
                % (name, name, len(prod.attributes)),
                1,
            )
        else:
            self.emit(
                f"if (!add_attributes(state, state->{name}_type, NULL, 0)) return 0;",
                1,
            )
    ...
    def visitSum(self, sum, name):
        self.emit(
            f'state->{name}_type = make_type(state, "{name}", state->AST_type, NULL, 0,',
            1,
        )
        self.emit(f"{reflow_c_string(asdl_of(name, sum), 2)});", 2, reflow=False)
        self.emit(f"if (!state->{name}_type) return 0;", 1)
        if sum.attributes:
            self.emit(
                "if (!add_attributes(state, state->%s_type, %s_attributes, %d)) return 0;"  # noqa: UP031
    ...
    def visitConstructor(self, cons, name, simple):
        if cons.fields:
            ...
            fields = "NULL"
        self.emit(
            'state->%s_type = make_type(state, "%s", state->%s_type, %s, %d,'  # noqa: UP031
            % (cons.name, cons.name, name, fields, len(cons.fields)),
            1,
        )
        self.emit(f"{reflow_c_string(asdl_of(cons.name, cons), 2)});", 2, reflow=False)
        self.emit(f"if (!state->{cons.name}_type) return 0;", 1)
        self.emit_defaults(cons.name, cons.fields, 1)
        if simple:
            self.emit(
                f"state->{cons.name}_singleton = PyType_GenericNew((PyTypeObject *)"
                f"state->{cons.name}_type, NULL, NULL);",
                1,
            )
            self.emit(f"if (!state->{cons.name}_singleton) return 0;", 1)

    def emit_defaults(self, name, fields, depth):
        for field in fields:
            if field.opt:
                self.emit(
                    f"if (PyObject_SetAttr(state->{name}_type, state->{field.name}, Py_None) == -1)",
                    depth,
                )
...
""".strip(),
    ...
    def addObj(self, name):
        self.emit(
            f'if (PyModule_AddObjectRef(m, "{name}", state->{name}_type) < 0) {{',
            1,
        )
        self.emit("return -1;", 2)
        self.emit("}", 1)
...
class ObjVisitor(PickleVisitor):
    def func_begin(self, name):
        ...
        self.emit(
            f"ast2obj_{name}(struct ast_state *state, struct validator *vstate, void* _o)",
            0,
        )
        self.emit("{", 0)
    ...
    def visitSum(self, sum, name):
        ...
        self.emit("}", 1)
        for a in sum.attributes:
            self.emit(f"value = ast2obj_{a.type}(state, vstate, o->{a.name});", 1)
            self.emit("if (!value) goto failed;", 1)
            self.emit(f"if (PyObject_SetAttr(result, state->{a.name}, value) < 0)", 1)
    ...
    def simpleSum(self, sum, name):
        self.emit(
            f"PyObject* ast2obj_{name}(struct ast_state *state, struct validator *vstate, {name}_ty o)",
            0,
        )
        self.emit("{", 0)
        ...
        for t in sum.types:
            ...
            self.emit(f"return Py_NewRef(state->{t.name}_singleton);", 3)
        self.emit("}", 1)
    ...
    def visitProduct(self, prod, name):
        self.func_begin(name)
        self.emit(f"tp = (PyTypeObject *)state->{name}_type;", 1)
    ...
    def visitConstructor(self, cons, enum, name):
        ...
        self.emit(f"tp = (PyTypeObject *)state->{cons.name}_type;", 2)
    ...
    def visitField(self, field, name, depth, product):
        ...
        if product:
            ...
            value = f"o->v.{name}.{field.name}"
        self.set(field, value, depth)
        emit("if (!value) goto failed;", 0)
        emit(f"if (PyObject_SetAttr(result, state->{field.name}, value) == -1)", 0)
        emit("goto failed;", 1)
    ...
    def set(self, field, value, depth):
        if field.seq:
            if field.type in self.metadata.simple_sums:
                ...
                self.emit("{", depth)
                self.emit(f"Py_ssize_t i, n = asdl_seq_LEN({value});", depth + 1)
                self.emit("value = PyList_New(n);", depth + 1)
                self.emit("if (!value) goto failed;", depth + 1)
                self.emit("for(i = 0; i < n; i++)", depth + 1)
                # This cannot fail, so no need for error handling
                self.emit(
                    f"PyList_SET_ITEM(value, i, ast2obj_{field.type}(state, vstate, ({field.type}_ty)asdl_seq_GET({value}, i)));",
                    depth + 2,
                    reflow=False,
                )
                self.emit("}", depth)
                ...
                self.emit(
                    f"value = ast2obj_list(state, vstate, (asdl_seq*){value}, ast2obj_{field.type});",
                    depth,
                )
        else:
            self.emit(
                f"value = ast2obj_{field.type}(state, vstate, {value});",
                depth,
                reflow=False,
            )
...
def generate_module_def(mod, metadata, f, internal_h):
    ...
    module_state.update(f"{singleton}_singleton" for singleton in metadata.singletons)
    module_state.update(f"{type}_type" for type in metadata.types)
