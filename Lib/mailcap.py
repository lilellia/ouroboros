...
def findmatch(caps, MIMEtype, key="view", filename="/dev/null", plist=None):
    ...
    if _find_unsafe(filename):
        msg = f"Refusing to use mailcap with filename {filename!r}. Use a safe temporary filename."
        ...
        if c != "%":
            ...
            if c == "%":
                ...
                if _find_unsafe(MIMEtype):
                    msg = f"Refusing to substitute MIME type {MIMEtype!r} into a shell command."
                    warnings.warn(msg, UnsafeMailcapInput)
                    return None
                res = res + MIMEtype
            elif c == "{":
