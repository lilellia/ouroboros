...
class Tokenizer:
    ...
    def checkgroupname(self, name, offset):
        if not (self.istext or name.isascii()):
            msg = f"bad character in group name {name!a}"
                ...
                if char == "P":
                    ...
                    if not (condname.isdecimal() and condname.isascii()):
                        ...
                        if not (condname.isdecimal() and condname.isascii()):
                            ...
                            warnings.warn(
                                "bad character in group name %s at position %d"  # noqa: UP031
                                % (
                                    repr(condname)
                                    if source.istext
                                    else ascii(condname),
...
def parse(str, flags=0, state=None):
    ...
    for g in p.state.grouprefpos:
        if g >= p.state.groups:
            msg = "invalid group reference %d" % g  # noqa: UP031
        ...
        if this[0] == "\\":
            ...
            if c == "g":
                ...
                if not (name.isdecimal() and name.isascii()):
                    ...
                    if index >= MAXGROUPS:
                        raise s.error(
                            "invalid group reference %d" % index, len(name) + 1  # noqa: UP031
                        )
                    if not (name.isdecimal() and name.isascii()):
                        import warnings
                ...
                try:
                    ...
                    if c in ASCIILETTERS:
                        raise s.error(f"bad escape {this}", len(this)) from None
                lappend(this)
        else:
            lappend(this)
    addliteral()
