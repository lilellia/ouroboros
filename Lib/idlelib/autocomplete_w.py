...
class AutoCompleteWindow:
    def __init__(self, widget, tags):
        ...
        # event ids
        self.hideid = self.keypressid = self.listupdateid = self.winconfigid = self.keyreleaseid = (
            self.doubleclickid
        ) = None
    ...
    def _change_start(self, newstart):
        ...
        if i < len(newstart):
            self.widget.insert("%s+%dc" % (self.startindex, i), newstart[i:], self.tags)  # noqa: UP031
        self.start = newstart
    ...
    def keypress_event(self, event):
        ...
        if (
            ...
            or (self.mode == FILES and keysym in ("slash", "backslash", "quotedbl", "apostrophe"))
            and not (state & ~MC_SHIFT)
        ):
