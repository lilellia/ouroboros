...
def _iter_bits_lsb(num):
    ...
    if isinstance(num, Enum):
        num = num.value
    if num < 0:
        raise ValueError(f"{original!r} is not a positive integer")
    while num:
        b = num & (~num + 1)
        yield b
        num ^= b
    ...
    def __set_name__(self, enum_class, member_name):
        ...
        try:
            try:
                ...
                for canonical_member in enum_class._member_map_.values():
    ...
    def __setitem__(self, key, value):
        ...
        if _is_private(self._cls_name, key):
            ...
            if key not in (
                ...
                raise ValueError(
                    f"_sunder_ names, such as {key!r}, are reserved for future Enum use"
                )
    ...
    def __new__(metacls, cls, bases, classdict, *, boundary=None, _simple=False, **kwds):
        ...
        if _order_:
            ...
            if _order_ != enum_class._member_names_:
                raise TypeError(
                    f"member order does not match _order_:\n  {enum_class._member_names_!r}\n  {_order_!r}"
                )
        return enum_class
...
class Enum(metaclass=EnumType):  # noqa: F811
    """
    ...
    def __new__(cls, value):
        ...
        if not cls._member_map_:
            raise TypeError(f"{cls!r} has no members defined")
        #
...
class StrEnum(str, ReprEnum):
    ...
    def __new__(cls, *values):
        ...
        if len(values) >= 2:  # noqa: SIM102
            # check that encoding argument is a string
            if not isinstance(values[1], str):
                raise TypeError(f"encoding must be a string, not {values[1]!r}")
        if len(values) == 3:  # noqa: SIM102
...
class Flag(Enum, boundary=STRICT):  # noqa: F811
    """
    Support for flags
    """
    ...
    def _missing_(cls, value):
        ...
        if member_value or aliases:
            ...
            if not combined_value:
                ...
                raise ValueError(f"{cls!r}: no members with value {unknown!r}")
            elif unknown:
    ...
    def __contains__(self, other):
        ...
        if not isinstance(other, self.__class__):
            raise TypeError(
                f"unsupported operand type(s) for 'in': {type(other).__qualname__!r} and {self.__class__.__qualname__!r}"
            )
...
def _dataclass_repr(self):
    ...
    return ", ".join(f"{k}={getattr(self, k)!r}" for k in dcf if dcf[k].repr)
...
def global_flag_repr(self):
    ...
    cls_name = self.__class__.__name__
    if self._name_ is None:
        return f"{module}.{cls_name}({self._value_!r})"
        ...
        for check in checks:
            if check is UNIQUE:
                ...
                if enum_type == "flag":
                    ...
                    raise Exception(f"verify: unknown type {enum_type!r}")  # noqa: TRY002
                if missing:
                    raise ValueError(
                        (
                            "invalid {} {!r}: missing values {}".format(
                                enum_type,
                                cls_name,
                ...
                if missing_names:
                    if len(missing_names) == 1:
                        ...
                        alias = "aliases {} and {} are missing".format(
                            ", ".join(missing_names[:-1]),
                            missing_names[-1],
                        )
                    if _is_single_bit(missing_value):
                        value = f"value 0x{missing_value:x}"
                    else:
                        value = f"combined values of 0x{missing_value:x}"
                    raise ValueError(
                        f"invalid Flag {cls_name!r}: {alias} {value} [use enum.show_flag_values(value) for details]"
                    )
        return enumeration
    ...
    if checked_enum.__dict__ != simple_enum.__dict__:
                ...
                if key == "__doc__":
                    ...
                    if compressed_checked_value != compressed_simple_value:
                        failed.append(
                            "{!r}:\n         {}\n         {}".format(
                                key,
                                f"checked -> {checked_value!r}",
                                f"simple  -> {simple_value!r}",
                            )
                        )
        ...
        for name in member_names:
            ...
            if name not in simple_keys:
                failed.append(f"missing member from simple enum: {name!r}")
            elif name not in checked_keys:
                failed.append(f"extra member in simple enum: {name!r}")
            else:
                    ...
                    if key in ("__module__", "__objclass__", "_inverted_"):
                        ...
                        if checked_value != simple_value:
                            failed_member.append(
                                "{!r}:\n         {}\n         {}".format(
                                    key,
                                    f"checked member -> {checked_value!r}",
                                    f"simple member  -> {simple_value!r}",
                                )
                            )
            if failed_member:
                failed.append(
            ...
            if method in simple_keys and method in checked_keys:
                ...
                if checked_method != simple_method:
                    failed.append(
                        ...
                        % (
                            method,
                            f"checked -> {checked_method!r}",
                            f"simple -> {simple_method!r}",
                        )
                    )
            else:
