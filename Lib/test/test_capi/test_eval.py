...
class Tests(unittest.TestCase):
    def test_eval_get_func_name(self):
        ...
        self.assertEqual(eval_get_func_name(function_example), "function_example")
        self.assertEqual(eval_get_func_name(A.method_example), "method_example")
        self.assertEqual(eval_get_func_name(A().method_example), "method_example")
    ...
    def test_eval_getlocals(self):
        ...
        x = 1
        self.assertEqual(_testcapi.eval_getlocals(), {"self": self, "x": 1})

        y = 2
        self.assertEqual(_testcapi.eval_getlocals(), {"self": self, "x": 1, "y": 2})

    def test_eval_getglobals(self):
        # Test PyEval_GetGlobals()
        self.assertEqual(_testcapi.eval_getglobals(), globals())

    def test_eval_getbuiltins(self):
        # Test PyEval_GetBuiltins()
        self.assertEqual(_testcapi.eval_getbuiltins(), globals()["__builtins__"])

    def test_eval_getframe(self):
        # Test PyEval_GetFrame()
        self.assertEqual(_testcapi.eval_getframe(), sys._getframe())

    def test_eval_get_recursion_limit(self):
        # Test Py_GetRecursionLimit()
        self.assertEqual(_testcapi.eval_get_recursion_limit(), sys.getrecursionlimit())
