...
class GenopsTests(unittest.TestCase):
    def test_genops(self):
        ...
        self.assertEqual(
            [(item[0].name,) + item[1:] for item in it],
            [
                ("MARK", None, 0),
                ("INT", 123, 1),
                ("BININT1", 0x12, 6),
    ...
    def test_no_stop(self):
        ...
        with self.assertRaisesRegex(ValueError, "pickle exhausted before seeing STOP"):
            next(it)
    ...
    def test_truncated_data(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "not enough data in stream to read int4"
        ):
            next(it)
    ...
    def test_unknown_opcode(self):
        ...
        self.assertEqual(item[0].name, "NONE")
        with self.assertRaisesRegex(
            ValueError, r"at position 1, opcode b'\\xff' unknown"
        ):
            next(it)
    ...
    def test_unknown_opcode_without_pos(self):
        ...
        self.assertEqual(item[0].name, "NONE")
        with self.assertRaisesRegex(
            ValueError, r"at position <unknown>, opcode b'\\xff' unknown"
        ):
            next(it)
