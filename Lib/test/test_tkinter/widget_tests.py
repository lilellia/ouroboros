...
class StandardOptionsTests:
    ...
    def test_configure_borderwidth(self):
        widget = self.create()
        self.checkPixelsParam(widget, "borderwidth", 0, 1.3, 2.6, 6, "10p")
        expected = 0 if self._clip_borderwidth else -2
        self.checkParam(
            widget, "borderwidth", -2, expected=expected, conv=self._conv_pixels
        )
        if "bd" in self.OPTIONS:
            self.checkPixelsParam(widget, "bd", 0, 1.3, 2.6, 6, "10p")
            self.checkParam(widget, "bd", -2, expected=expected, conv=self._conv_pixels)
    ...
    def test_configure_highlightthickness(self):
        ...
        self.checkParam(
            widget, "highlightthickness", -2, expected=expected, conv=self._conv_pixels
        )
    ...
    def test_configure_justify(self):
        ...
        if self._allow_empty_justify:
            values += ("",)
        self.checkEnumParam(widget, "justify", *values, fullname="justification")
    ...
    def test_configure_padx(self):
        ...
        self.checkPixelsParam(
            widget, "padx", 3, 4.4, 5.6, "12m", conv=self._conv_pad_pixels
        )
        expected = 0 if self._clip_pad else -2
        self.checkParam(
            widget, "padx", -2, expected=expected, conv=self._conv_pad_pixels
        )

    def test_configure_pady(self):
        widget = self.create()
        self.checkPixelsParam(
            widget, "pady", 3, 4.4, 5.6, "12m", conv=self._conv_pad_pixels
        )
        expected = 0 if self._clip_pad else -2
        self.checkParam(
            widget, "pady", -2, expected=expected, conv=self._conv_pad_pixels
        )
    ...
    def test_configure_underline(self):
        ...
        if tk_version >= (8, 7):
            is_ttk = widget.__class__.__module__ == "tkinter.ttk"
            self.checkParam(
                widget, "underline", "", expected="" if is_ttk else self._default_pixels
            )
            self.checkParam(widget, "underline", "5+2", expected="5+2" if is_ttk else 7)
            self.checkParam(widget, "underline", "5-2", expected="5-2" if is_ttk else 3)
            self.checkParam(widget, "underline", "end", expected="end")
            self.checkParam(widget, "underline", "end-2", expected="end-2")
            errmsg = (
                r'bad index "{}": must be integer\?\[\+-\]integer\?, '
                r'end\?\[\+-\]integer\?, or ""'
            )
        else:
            errmsg = 'expected integer but got "{}"'
    ...
    def test_configure_overrelief(self):
        widget = self.create()
        self.checkReliefParam(widget, "overrelief", allow_empty=(tk_version >= (8, 7)))
