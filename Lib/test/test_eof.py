...
class EOFTestCase(unittest.TestCase):
    ...
    def test_EOFS(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "unterminated triple-quoted string literal (detected at line 4) (<string>, line 2)",
        )
        self.assertEqual(cm.exception.text, "ä = '''thîs is ")
        self.assertEqual(cm.exception.offset, 5)
    ...
    def test_EOFS_with_file(self):
        ...
        with os_helper.temp_dir() as temp_dir:
            ...
            file_name = script_helper.make_script(
                temp_dir, "foo", BOM_UTF8 + """ä = '''thîs is \na \ntest""".encode()
            )
            rc, out, err = script_helper.assert_python_failure("-X", "utf8", file_name)
            err = err.decode().splitlines()
            self.assertEqual(
    ...
    def test_line_continuation_EOF(self):
        ...
        with self.assertRaises(SyntaxError) as cm:
            exec("# coding:latin1\nä = 5\\".encode("latin1"))
        self.assertEqual(
            str(cm.exception), "unexpected EOF while parsing (<string>, line 2)"
        )
        self.assertEqual(cm.exception.text, "ä = 5\\\n")
        self.assertEqual(cm.exception.offset, 7)
    ...
    def test_line_continuation_EOF_from_file_bpo2180(self):
        ...
        with os_helper.temp_dir() as temp_dir:
            ...
            err = err.decode().splitlines()
            self.assertEqual(
                err[-2:], ["    \\", "SyntaxError: unexpected EOF while parsing"]
            )
            ...
            file_name = script_helper.make_script(
                temp_dir, "foo", BOM_UTF8 + "ä = 8\\".encode()
            )
            rc, out, err = script_helper.assert_python_failure("-X", "utf8", file_name)
            err = err.decode().splitlines()
            self.assertEqual(
