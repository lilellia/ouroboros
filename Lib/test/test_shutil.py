...
class TestArchives(BaseTest, unittest.TestCase):
    ...
    def test_make_tarfile(self):
        ...
        with os_helper.temp_cwd(), no_chdir:
            ...
            archive = make_archive(
                base_name, "tar", root_dir, os.path.join(base_dir, "sub")
            )
            self.assertEqual(archive, os.path.abspath(base_name) + ".tar")
            self.assertTrue(os.path.isfile(archive))
            self.assertTrue(tarfile.is_tarfile(archive))
            with tarfile.open(archive, "r") as tf:
                self.assertCountEqual(tf.getnames(), ["dist/sub", "dist/sub/file3"])
    ...
    def test_make_zipfile(self):
        ...
        with os_helper.temp_cwd(), no_chdir:
            base_name = os.path.join("dst3", "archive")
            archive = make_archive(
                base_name, "zip", root_dir, os.path.join(base_dir, "sub")
            )
            self.assertEqual(archive, os.path.abspath(base_name) + ".zip")
            self.assertTrue(os.path.isfile(archive))
            self.assertTrue(zipfile.is_zipfile(archive))
            with zipfile.ZipFile(archive) as zf:
                self.assertCountEqual(zf.namelist(), ["dist/sub/", "dist/sub/file3"])
    ...
    def test_unpack_archive_zip_badpaths(self):
        ...
        if os.name != "nt":
            ...
            self.assertTrue(
                os.path.isfile(os.path.join(dstdir, "\\\\server2\\share\\abspath"))
            )
        if os.pardir == "..":
...
class TestWhich(BaseTest, unittest.TestCase):
    ...
    def test_empty_path(self):
        base_dir = os.path.dirname(self.dir)
        with (
            os_helper.change_cwd(path=self.dir),
    ...
    def test_pathext_extension_ends_with_dot(self):
        ...
        with os_helper.EnvironmentVarGuard() as env:
            env["PATHEXT"] = ext + "."
            self.assertEqual(
                shutil.which(cmd, path=self.dir), filepath
            )  # cmd.exe hangs here
            self.assertEqual(shutil.which(cmdext, path=self.dir), filepath)
    ...
    def test_same_dir_with_pathext_extension(self):
        ...
        self.assertNormEqual(
            shutil.which(cmd, path=self.dir, mode=os.F_OK), self.filepath
        )
        ...
        self.assertNormEqual(
            shutil.which(cmd2, path=self.dir, mode=os.F_OK), other_file_path
        )
    ...
    def test_same_dir_without_pathext_extension(self):
        ...
        # full match
        self.assertNormEqual(
            shutil.which(cmd, path=self.dir, mode=os.F_OK), other_file_path
        )
        self.assertNormEqual(shutil.which(self.file, path=self.dir), self.filepath)
        self.assertNormEqual(
            shutil.which(self.file, path=self.dir, mode=os.F_OK), self.filepath
        )
    ...
    def test_dir_order_with_pathext_extension(self):
        ...
        search_path = os.pathsep.join(
            [os.fsdecode(self.other_dir), os.fsdecode(self.dir)]
        )
        # full match in the second directory
        self.assertNormEqual(shutil.which(cmd, path=search_path), self.filepath)
        self.assertNormEqual(
            shutil.which(cmd, path=search_path, mode=os.F_OK), self.filepath
        )
        ...
        self.assertNormEqual(
            shutil.which(cmd, path=search_path, mode=os.F_OK), other_file_path
        )
        # full match in the first directory
        self.assertNormEqual(shutil.which(cmd2, path=search_path), other_file_path)
        self.assertNormEqual(
            shutil.which(cmd2, path=search_path, mode=os.F_OK), other_file_path
        )

        # full match in the first directory
        search_path = os.pathsep.join(
            [os.fsdecode(self.dir), os.fsdecode(self.other_dir)]
        )
    ...
    def test_dir_order_without_pathext_extension(self):
        ...
        self.assertNormEqual(
            shutil.which(self.file, path=search_path, mode=os.F_OK), self.filepath
        )
