...
class StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.TestCase):
    ...
    def make_zip64_file(
        ...
        extensible_data=b"",
        end_of_central_dir_size=None,
        offset_to_end_of_central_dir=None,
    ):
        ...
        if offset_to_end_of_central_dir is None:
            offset_to_end_of_central_dir = (
                108 + 8 * len(local_zip64_fields) + 8 * len(central_zip64_fields)
            )
    ...
    def test_zip64_end_of_central_dir_record_not_found(self):
        ...
        zipdata = self.make_zip64_file(extensible_data=b"\xca\xfe\x04\x00\x00\x00data")
        zipdata = zipdata.replace(b"PK\x06\x06", b"\x00" * 4)
        with self.assertRaisesRegex(zipfile.BadZipFile, "record not found"):
            zipfile.ZipFile(io.BytesIO(zipdata))
        self.assertFalse(zipfile.is_zipfile(io.BytesIO(zipdata)))
