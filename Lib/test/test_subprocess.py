...
class ProcessTestCase(BaseTestCase):
    ...
    def test_check_output_timeout(self):
        ...
        with self.assertRaises(subprocess.TimeoutExpired) as c:
            output = subprocess.check_output(
                [sys.executable, "-c", "import time; time.sleep(3600)"], timeout=0.1
            )
    ...
    @unittest.skipIf(
        sysconfig.get_config_var("Py_ENABLE_SHARED") == 1,
        "The Python shared library cannot be loaded without some system environments.",
    )
    @unittest.skipIf(
        check_sanitizer(address=True), "AddressSanitizer adds to the environment."
    )
...
class RunFuncTestCase(BaseTestCase):
    ...
    def test_check_output_timeout(self):
        with self.assertRaises(subprocess.TimeoutExpired) as c:
            cp = self.run_python(
                "import time; time.sleep(3600)", timeout=0.1, stdout=subprocess.PIPE
            )
