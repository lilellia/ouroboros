...
class PosixPathTest(unittest.TestCase):
    ...
    def test_expanduser_pwd2(self):
        ...
        for all_entry in pwd.getpwall():
            ...
            with self.subTest(all_entry=all_entry, entry=entry):
                ...
                self.assertEqual(
                    posixpath.expanduser(os.fsencode("~" + name)), os.fsencode(home)
                )
    ...
    def test_realpath_symlink_loops_strict(self, kwargs):
        ...
        try:
            ...
            os.symlink(ABSTFN + "x", ABSTFN + "y")
            self.assertRaises(
                OSError, realpath, ABSTFN + "1/../" + basename(ABSTFN) + "y", **kwargs
            )
            self.assertRaises(
                OSError, realpath, ABSTFN + "1/../" + basename(ABSTFN) + "1", **kwargs
            )
    ...
    def test_realpath_resolve_parents(self, kwargs):
        ...
        try:
            ...
            with os_helper.change_cwd(ABSTFN + "/k"):
                self.assertEqual(realpath("a", **kwargs), ABSTFN + "/y/a")
        finally:
    ...
    def test_realpath_resolve_before_normalizing(self, kwargs):
        ...
        try:
            ...
            with os_helper.change_cwd(dirname(ABSTFN)):
                self.assertEqual(
                    realpath(basename(ABSTFN) + "/link-y/..", **kwargs), ABSTFN + "/k"
                )
        finally:
    ...
    @skip_if_ABSTFN_contains_backslash
    @unittest.skipIf(
        os.chmod not in os.supports_follow_symlinks, "Can't set symlink permissions"
    )
    @unittest.skipIf(
        sys.platform != "darwin", "only macOS requires read permission to readlink()"
    )
    @_parameterize({"strict": True}, {"strict": ALLOW_MISSING})
    ...
    def test_realpath_unreadable_directory(self):
        try:
            ...
            self.assertEqual(realpath(ABSTFN + "/k", strict=False), ABSTFN + "/k")
            self.assertRaises(PermissionError, realpath, ABSTFN + "/k", strict=True)
            self.assertRaises(
                PermissionError, realpath, ABSTFN + "/k", strict=ALLOW_MISSING
            )

            self.assertEqual(
                realpath(ABSTFN + "/missing", strict=False), ABSTFN + "/missing"
            )
            self.assertRaises(
                PermissionError, realpath, ABSTFN + "/missing", strict=True
            )
            self.assertRaises(
                PermissionError, realpath, ABSTFN + "/missing", strict=ALLOW_MISSING
            )
        finally:
