...
class ArgsTestCase(BaseTestCase):
    ...
    def test_leak_tmp_file(self):
        ...
        for testname in testnames:
            self.assertIn(
                f"Warning -- {testname} leaked temporary files (1): mytmpfile", output
            )
...
class TestUtils(unittest.TestCase):
    ...
    def test_sanitize_xml(self):
        ...
        self.assertEqual(sanitize_xml("nul:\x00, bell:\x07"), r"nul:\x00, bell:\x07")
        self.assertEqual(sanitize_xml("surrogate:\udc80"), r"surrogate:\udc80")
        self.assertEqual(
            sanitize_xml("illegal \ufffe and \uffff"), r"illegal \ufffe and \uffff"
        )

        # no escape for valid XML characters
        self.assertEqual(sanitize_xml("a\n\tb"), "a\n\tb")
        self.assertEqual(sanitize_xml("valid t\xe9xt \u20ac"), "valid t\xe9xt \u20ac")
