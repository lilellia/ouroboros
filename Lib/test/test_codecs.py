        ...
        for i in range(97, 123):
            ...
            if b not in b"abfnrtvx":
                with self.assertWarnsRegex(
                    DeprecationWarning, r"invalid escape sequence '\\%c'" % i
                ):
                    check(b"\\" + b, b"\\" + b)
            with self.assertWarnsRegex(
                DeprecationWarning, r"invalid escape sequence '\\%c'" % (i - 32)
            ):
                check(b"\\" + b.upper(), b"\\" + b.upper())
        with self.assertWarnsRegex(
            DeprecationWarning, r"invalid escape sequence '\\8'"
        ):
            check(rb"\8", b"\\8")
        with self.assertWarns(DeprecationWarning):
            check(rb"\9", b"\\9")
        with self.assertWarnsRegex(
            DeprecationWarning, r"invalid escape sequence '\\\xfa'"
        ) as cm:
            check(b"\\\xfa", b"\\\xfa")
        for i in range(0o400, 0o1000):
            with self.assertWarnsRegex(
                DeprecationWarning, r"invalid octal escape sequence '\\%o'" % i
            ):
                check(rb"\%o" % i, bytes([i & 0o377]))

        with self.assertWarnsRegex(
            DeprecationWarning, r"invalid escape sequence '\\z'"
        ):
            self.assertEqual(decode(rb"\x\z", "ignore"), (b"\\z", 4))
        with self.assertWarnsRegex(
            DeprecationWarning, r"invalid octal escape sequence '\\501'"
        ):
            self.assertEqual(decode(rb"\x\501", "ignore"), (b"A", 6))
...
class UnicodeEscapeTest(ReadTest, unittest.TestCase):
    ...
    def test_decode_warnings(self):
        ...
        with self.assertWarnsRegex(
            ...
            self.assertEqual(decode(rb"\x\z", "ignore"), ("\\z", 4))
        with self.assertWarnsRegex(
            DeprecationWarning, r"invalid octal escape sequence '\\501'"
        ):
            self.assertEqual(decode(rb"\x\501", "ignore"), ("\u0141", 6))
