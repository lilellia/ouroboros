...
class urlopen_DataTests(unittest.TestCase):
    ...
    def test_info(self):
        ...
        self.assertEqual(
            r.info().get_params(), [("text/plain", ""), ("charset", "US-ASCII")]
        )
        r.close()
    ...
    def test_invalid_mediatype(self):
        for c0 in control_characters_c0():
            self.assertRaises(
                ValueError, urllib.request.urlopen, f"data:text/html;{c0},data"
            )
        for c0 in control_characters_c0():
            self.assertRaises(
                ValueError,
...
class Pathname_Tests(unittest.TestCase):
    ...
    @unittest.skipUnless(sys.platform == "win32", "test specific to Windows pathnames.")
    def test_pathname2url_win(self):
        ...
        self.assertEqual(
            fn("\\\\some\\share\\a\\b%#c\xe9"), "//some/share/a/b%25%23c%C3%A9"
        )
        ...
        self.assertEqual(fn("//?/unc/server/share/dir"), "//server/share/dir")
        # Round-tripping
        urls = ["///C:", "/folder/test/", "///C:/foo/bar/spam.foo"]
        for url in urls:
            self.assertEqual(fn(urllib.request.url2pathname(url)), url)

    @unittest.skipIf(sys.platform == "win32", "test specific to POSIX pathnames")
    ...
    def test_pathname2url_nonascii(self):
        ...
        url = urllib.parse.quote(
            os_helper.FS_NONASCII, encoding=encoding, errors=errors
        )
    ...
    def test_url2pathname_win(self):
        ...
        self.assertEqual(
            fn("//localhost//server/path/to/file"), "\\\\server\\path\\to\\file"
        )
        ...
        # Round-tripping
        paths = ["C:", r"\C\test\\", r"C:\foo\bar\spam.foo"]
        for path in paths:
