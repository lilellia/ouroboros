...
class ThreadedTests(unittest.TestCase):
    ...
    def test_dh_params(self):
        ...
        if not (
            supports_kx_alias(client_context, dhe_aliases)
...
class TestPreHandshakeClose(unittest.TestCase):
    ...
    def non_linux_skip_if_other_okay_error(self, err):
        ...
        if (
            ...
            or re.search("wrong.version.number", str(getattr(err, "reason", "")), re.I)
        ):
            ...
            try:
                self.skipTest(
                    f"Could not recreate conditions on {sys.platform}: {err=}"
                )
            finally:
