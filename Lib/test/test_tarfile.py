    ...
    def test_pax_header_bad_formats(self):
        ...
        for replacement in pax_header_replacements:
            with self.subTest(header=replacement):
                ...
                with self.assertRaisesRegex(
                    tarfile.ReadError, r"method tar: ReadError\('invalid header'\)"
                ):
    ...
    def test_deferred_directory_attributes_update(self, mock_chmod):
        ...
        def mock_chmod_side_effect(path, mode, **kwargs):
            ...
            if os.path.commonpath([target_path, tempdir]) != tempdir:
                raise Exception(
                    "should not try to chmod anything outside the destination",
                    target_path,
                )
...
class TestExtractionFilters(unittest.TestCase):
    ...
    def expect_file(
        self, name, type=None, symlink_to=None, mode=None, size=None, content=None
    ):
    ...
    def expect_any_tree(self, name):
        ...
        self.expected_paths = {
            p for p in self.expected_paths if tree_path not in p.parents
        }
    ...
    def test_realpath_limit_attack(self):
        ...
        with ArchiveMaker() as arc:
            ...
            if sys.platform == "win32":
                ...
                raise NotImplementedError(
                    "Need to guess component length for {sys.platform}"
                )
            path = ""
            step_path = ""
            for i in steps:
        ...
        with (
            self.subTest("fully_trusted"),
            self.check_context(arc.open(), filter="fully_trusted", check_flag=False),
        ):
            if sys.platform == "win32":
    ...
    def test_sneaky_hardlink_fallback(self):
        ...
        with ArchiveMaker() as arc:
            ...
            arc.add("c/escape", symlink_to=os.path.join("..", "..", "link_here"))
        ...
        with self.check_context(arc.open(), "data"):
            if not os_helper.can_symlink():
                ...
                self.assertIsInstance(e.__cause__, tarfile.LinkOutsideDestinationError)
    ...
    def test_exfiltration_via_symlink(self):
        ...
        with ArchiveMaker() as arc:
            arc.add(
                "escape",
                symlink_to=os.path.join("link", "link", "..", "..", "link-here"),
            )
            ...
            with self.check_context(arc.open(), filter):
                if os_helper.can_symlink():
                    ...
                    if filter == "data":
                        ...
                        self.expect_file(
                            "escape", symlink_to="link/link/../../link-here"
                        )
                else:
    ...
    def test_chown_links_on_extract(self, link_type):
        with ArchiveMaker() as arc:
            arc.add(
                "test.txt", uid=1337, gid=1337, uname="", gname="", mode="-rwxr-xr-x"
            )
            arc.add(
                "link",
                type=link_type,
...
class OffsetValidationTests(unittest.TestCase):
    ...
    def _get_members(self, ignore_zeros=None):
        with open(self.tarname, "rb") as f:
            with tarfile.open(mode="r", fileobj=f, ignore_zeros=ignore_zeros) as tar:
