...
class TestNtpath(NtpathTestCase):
    ...
    def test_realpath_curdir_missing_ok(self):
        ...
        tester("ntpath.realpath('.\\.', strict=ALLOW_MISSING)", expected)
        tester(
            "ntpath.realpath('\\'.join(['.'] * 100), strict=ALLOW_MISSING)", expected
        )
    ...
    def test_realpath_pardir_strict(self):
        ...
        tester(
            "ntpath.realpath('../..', strict=True)",
            ntpath.dirname(ntpath.dirname(expected)),
        )
        tester(
            "ntpath.realpath('/'.join(['..'] * 50), strict=True)",
            ntpath.splitdrive(expected)[0] + "\\",
        )
        tester(
            "ntpath.realpath('..\\..', strict=True)",
    ...
    def test_realpath_pardir_missing_ok(self):
        ...
        tester(
            "ntpath.realpath('../..', strict=ALLOW_MISSING)",
            ntpath.dirname(ntpath.dirname(expected)),
        )
        tester(
            "ntpath.realpath('/'.join(['..'] * 50), strict=ALLOW_MISSING)",
            ntpath.splitdrive(expected)[0] + "\\",
        )
        tester(
            "ntpath.realpath('..\\..', strict=ALLOW_MISSING)",
    ...
    def test_realpath_basic(self, kwargs):
        ...
        self.assertPathEqual(
            ntpath.realpath(os.fsencode(ABSTFN + "1"), **kwargs), os.fsencode(ABSTFN)
        )
    ...
    def test_realpath_symlink_loops_raise(self):
        ...
        os.symlink(ABSTFN + "1", ABSTFN + "2")
        os.symlink(ABSTFN + "2", ABSTFN + "1")
        self.assertRaises(OSError, ntpath.realpath, ABSTFN + "1", strict=ALLOW_MISSING)
        self.assertRaises(OSError, ntpath.realpath, ABSTFN + "2", strict=ALLOW_MISSING)
        self.assertRaises(
            OSError, ntpath.realpath, ABSTFN + "1\\x", strict=ALLOW_MISSING
        )
        ...
        os.symlink(ntpath.basename(ABSTFN) + "a\\b", ABSTFN + "a")
        self.assertRaises(OSError, ntpath.realpath, ABSTFN + "a", strict=ALLOW_MISSING)
        ...
        self.assertRaises(OSError, ntpath.realpath, ABSTFN + "c", strict=ALLOW_MISSING)

        # Test using relative path as well.
        self.assertRaises(
            OSError, ntpath.realpath, ntpath.basename(ABSTFN), strict=ALLOW_MISSING
        )
    ...
    def test_realpath_symlink_prefix(self, kwargs):
        ...
        self.assertPathEqual(ntpath.realpath(ABSTFN + "3link", **kwargs), ABSTFN + "3")
        self.assertPathEqual(
            ntpath.realpath(ABSTFN + "3.link", **kwargs), "\\\\?\\" + ABSTFN + "3."
        )
        ...
        self.assertPathEqual(
            ntpath.realpath("\\\\?\\" + ABSTFN + "3link", **kwargs),
            "\\\\?\\" + ABSTFN + "3",
        )
        self.assertPathEqual(
            ntpath.realpath("\\\\?\\" + ABSTFN + "3.link", **kwargs),
            "\\\\?\\" + ABSTFN + "3.",
        )
        ...
        for kwargs in {}, {"strict": True}, {"strict": ALLOW_MISSING}:
            with self.subTest(**kwargs):
                with os_helper.change_cwd(test_dir_long):
                    self.assertPathEqual(
                        test_file_long, ntpath.realpath("file.txt", **kwargs)
                    )
    ...
    def test_abspath(self):
        ...
        self.assertEqual(
            ntpath.abspath("C:\x00"), ntpath.join(ntpath.abspath("C:"), "\x00")
        )
    ...
    def test_isfile_named_pipe(self):
        ...
        h = _winapi.CreateNamedPipe(
            named_pipe, _winapi.PIPE_ACCESS_INBOUND, 0, 1, 0, 0, 0, 0
        )
        try:
