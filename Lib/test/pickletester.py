...
from test.support import TestFailed, run_with_locales, no_tracing, _2G, _4G, bigmemtest
...
class AbstractUnpickleTests:
    ...
    def test_load_global(self):
        ...
        with support.swap_item(sys.modules, "mödule", types.SimpleNamespace(glöbal=42)):
            self.assertEqual(
                self.loads(b"\x80\x04cm\xc3\xb6dule\ngl\xc3\xb6bal\n."), 42
            )
    ...
    def test_load_stack_global(self):
        ...
        self.assertIs(self.loads(b"\x8c\x07os.path\x8c\x04join\x93."), os.path.join)
        self.assertIs(
            self.loads(b"\x80\x04\x8c\x08builtins\x8c\x09str.upper\x93."), str.upper
        )
        with support.swap_item(sys.modules, "mödule", types.SimpleNamespace(glöbal=42)):
            self.assertEqual(
                self.loads(b"\x80\x04\x8c\x07m\xc3\xb6dule\x8c\x07gl\xc3\xb6bal\x93."),
                42,
            )

        self.assertRaises(
            UnicodeDecodeError, self.loads, b"\x8c\x01\xff\x8c\x03log\x93."
        )
        self.assertRaises(
            UnicodeDecodeError, self.loads, b"\x8c\x04math\x8c\x01\xff\x93."
        )
        ...
        self.assertRaises(
            AttributeError, self.loads, b"\x80\x04\x8c\x04math\x8c\x00\x93."
        )
        ...
        self.assertRaises(
            pickle.UnpicklingError, self.loads, b"\x80\x04\x8c\x04mathN\x93."
        )
    ...
    def test_find_class(self):
        ...
        self.assertRaises(
            ModuleNotFoundError, unpickler_nofix.find_class, "__builtin__", "str"
        )
        ...
        self.assertRaises(
            (TypeError, AttributeError), unpickler4.find_class, "math", None
        )
    ...
    def test_bad_state(self):
        ...
        self.check_unpickling_error(
            CustomError, base + b"}c__main__\nBadKey1\n)\x81Nsb."
        )
...
class AbstractPicklingErrorTests:
    ...
    def test_bad_ext_code(self):
        ...
        def check(code, exc):
            ...
            with (
                support.swap_item(copyreg._extension_registry, key, code),
    ...
    def test_bytes_memoization(self):
        for proto in protocols:
            for array_type in [bytes, ZeroCopyBytes]:
                for s in b"", b"xyz", b"xyz" * 100:
                    with self.subTest(
                        proto=proto, array_type=array_type, s=s, independent=False
                    ):
                        b = array_type(s)
                    ...
                    with self.subTest(
                        proto=proto, array_type=array_type, s=s, independent=True
                    ):
...
class AbstractIdentityPersistentPicklerTests:
    ...
    def test_protocol0_is_ascii_only(self):
        ...
        self.assertEqual(
            str(cm.exception), "persistent IDs in protocol 0 must be ASCII strings"
        )
