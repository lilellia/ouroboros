...
class ConfigDictTest(BaseTest):
    ...
    def test_config_queue_handler_does_not_create_multiprocessing_manager(
        self, manager
    ):
    ...
    @patch("multiprocessing.Manager")
    def test_config_queue_handler_invalid_config_does_not_create_multiprocessing_manager(
        self, manager
    ):
        # gh-120868, gh-121723
    ...
    @unittest.skipUnless(
        support.Py_DEBUG,
        "requires a debug build for testing assertions in multiprocessing",
    )
    ...
    @unittest.skipUnless(
        support.Py_DEBUG,
        "requires a debug build for testingassertions in multiprocessing",
    )
        ...
        for start_method in start_methods:
            with self.subTest(start_method=start_method):
                ...
                with ctx.Manager() as manager:
                    ...
                    with ctx.Pool(
                        1,
                        initializer=self._mpinit_issue121723,
                        initargs=(q, "text"),
                        maxtasksperchild=1,
                    ):
    ...
    def test_kwargs_passing(self):
        ...
        config = {
            "version": 1,
            "handlers": {
                "custom": {"class": CustomQueueHandler, **custom_kwargs},
            },
    ...
    def test_taskName_without_asyncio_imported(self):
        try:
            ...
            with (
                ...
                support.swap_item(sys.modules, "asyncio", None),
            ):
                logging.logAsyncioTasks = True
                runner.run(make_record(self.assertIsNone))
                logging.logAsyncioTasks = False
