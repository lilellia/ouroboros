...
class TestInteractiveConsole(unittest.TestCase):
    ...
    def test_syntax_error(self):
        self.infunc.side_effect = ["def f():", "    x = ?", "", EOFError("Finished")]
        self.console.interact()
    ...
    def test_sysexcepthook(self):
        ...
        hook.assert_called_with(
            self.sysmod.last_type, self.sysmod.last_value, self.sysmod.last_traceback
        )
    ...
    def test_sysexcepthook_crashing_doesnt_close_repl(self):
        ...
        error = "".join(
            call.args[0] for call in self.stderr.method_calls if call[0] == "write"
        )
