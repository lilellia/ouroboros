    ...
    def test_mutating_decode_handler_unicode_escape(self):
        ...
        def check(input, expected, msg):
            with self.assertWarns(DeprecationWarning) as cm:
                self.assertEqual(
                    decode(input, "test.mutating2"), (expected, len(input))
                )
            self.assertIn(msg, str(cm.warning))

        check(rb"\x0n\z", "\u0404\n\\z", r"invalid escape sequence '\z'")
        check(rb"\x0n\501", "\u0404\n\u0141", r"invalid octal escape sequence '\501'")
        check(rb"\x0z", "\u0404\\z", r"invalid escape sequence '\z'")

        check(rb"\x3n\zr", "\u0404\n\\zr", r"invalid escape sequence '\z'")
        check(rb"\x3zr", "\u0404\\zr", r"invalid escape sequence '\z'")
        check(rb"\x3z5", "\u0404\\z5", r"invalid escape sequence '\z'")
        check(
            memoryview(rb"\x3z5x")[:-1], "\u0404\\z5", r"invalid escape sequence '\z'"
        )
        check(
            memoryview(rb"\x3z5xy")[:-2], "\u0404\\z5", r"invalid escape sequence '\z'"
        )

        check(rb"\x5n\z", "\u0404\n\\z", r"invalid escape sequence '\z'")
        check(rb"\x5n\501", "\u0404\n\u0141", r"invalid octal escape sequence '\501'")
        check(rb"\x5z", "\u0404\\z", r"invalid escape sequence '\z'")
        check(memoryview(rb"\x5zy")[:-1], "\u0404\\z", r"invalid escape sequence '\z'")
