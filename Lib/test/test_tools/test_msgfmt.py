...
class CompilationTest(unittest.TestCase):
    ...
    def test_translations(self):
        ...
        self.assertEqual(
            t.ngettext("One email sent.", "%d emails sent.", 1), "One email sent."
        )
        self.assertEqual(
            t.ngettext("One email sent.", "%d emails sent.", 2), "%d emails sent."
        )
        self.assertEqual(
            t.npgettext("abc", "One email sent.", "%d emails sent.", 1),
            "One email sent.",
        )
        self.assertEqual(
            t.npgettext("abc", "One email sent.", "%d emails sent.", 2),
            "%d emails sent.",
        )
...
class CLITest(unittest.TestCase):
    ...
    def test_invalid_option(self):
        ...
        err = res.err.decode("utf-8")
        self.assertIn(
            "Generate binary message catalog from textual translation description.", err
        )
