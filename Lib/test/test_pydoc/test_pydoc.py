...
class PydocDocTest(unittest.TestCase):
    ...
    def test_help_output_redirect(self, pager_mock):
        ...
        with (
            captured_output("stdout") as output,
    ...
    def test_help_output_redirect_various_requests(self, pager_mock):
        ...
        def run_pydoc_for_request(request, expected_text_part):
            ...
            with (
                ...
                result = buf.getvalue().strip()
                self.assertEqual(
                    "", output.getvalue(), msg=f'failed on request "{request}"'
                )
                self.assertEqual(
                    "", err.getvalue(), msg=f'failed on request "{request}"'
                )
                self.assertIn(
                    expected_text_part, result, msg=f'failed on request "{request}"'
                )
                pager_mock.assert_not_called()
        ...
        run_pydoc_for_request(
            "pydoc.Helper.help", "Help on function help in pydoc.Helper:"
        )
        # test for pydoc.Helper.help
        run_pydoc_for_request(
            pydoc.Helper.help, "Help on function help in module pydoc:"
        )
    ...
    def test_source_synopsis(self):
        ...
        check('"""Embedded\0null byte"""', None)
        check('"""Embedded null byte"""\0', None)
