...
def get_preferred_scheme(key):
    ...
    if scheme not in _INSTALL_SCHEMES:
        raise ValueError(
            f"{key!r} returned {scheme!r}, which is not a valid scheme on this platform"
        )
    return scheme
...
def get_paths(scheme=get_default_scheme(), vars=None, expand=True):  # noqa: B008
...
def get_path(name, scheme=get_default_scheme(), vars=None, expand=True):  # noqa: B008
