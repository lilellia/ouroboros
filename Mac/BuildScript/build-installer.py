...
def grepValue(fn, variable):
    ...
    """
    variable = variable + "="
    for ln in open(fn, "r"):  # noqa: SIM115
        if ln.startswith(variable):
            value = ln[len(variable) :].strip()
            return value.strip("\"'")
    raise RuntimeError(f"Cannot find variable {variable[:-1]}")
            ...
            [
                {
                    ...
                    "install": "make TCL_LIBRARY={TCL_LIBRARY} && make install TCL_LIBRARY={TCL_LIBRARY} DESTDIR={DESTDIR}".format(
                        DESTDIR=shellQuote(os.path.join(WORKDIR, "libraries")),
                        TCL_LIBRARY=shellQuote(
                            f"/Library/Frameworks/Python.framework/Versions/{getVersion()}/lib/tcl8.6"
                        ),
                    ),
                },
                {
                    ...
                    "useLDFlags": False,
                    "install": "make TCL_LIBRARY={TCL_LIBRARY} TK_LIBRARY={TK_LIBRARY} && make install TCL_LIBRARY={TCL_LIBRARY} TK_LIBRARY={TK_LIBRARY} DESTDIR={DESTDIR}".format(
                        DESTDIR=shellQuote(os.path.join(WORKDIR, "libraries")),
                        TCL_LIBRARY=shellQuote(
                            f"/Library/Frameworks/Python.framework/Versions/{getVersion()}/lib/tcl8.6"
                        ),
                        TK_LIBRARY=shellQuote(
                            f"/Library/Frameworks/Python.framework/Versions/{getVersion()}/lib/tk8.6"
                        ),
                    ),
                },
            ]
        ...
        [
            {
                ...
                "useLDFlags": False,
                "install": "make && make install DESTDIR={} && cd {}/usr/local/lib && ln -fs ../../../Library/Frameworks/Python.framework/Versions/{}/lib/lib* .".format(
...
def runCommand(commandline):
    ...
    if xit is not None:
        ...
        raise RuntimeError(f"command failed: {commandline}")
...
def getTclTkVersion(configfile, versionline):
    ...
    """
    try:
        f = open(configfile, "r")  # noqa: SIM115
    except OSError:
        fatal(f"Framework configuration file not found: {configfile}")
...
def checkEnvironment():
    ...
    if not internalTk():
        ...
        for framework in ["Tcl", "Tk"]:
            fwpth = f"Library/Frameworks/{framework}.framework/Versions/Current"
            libfw = os.path.join("/", fwpth)
            usrfw = os.path.join(os.getenv("HOME"), fwpth)
            frameworks[framework] = os.readlink(libfw)
            if not os.path.exists(libfw):
                fatal(
                    f"Please install a link to a current {framework} {frameworks[framework]} as {libfw} so "
                    "the user can override the system framework."
                )
            if os.path.exists(usrfw):
                fatal(f"Please rename {usrfw} to avoid possible dynamic load issues.")
    ...
    for ev in list(os.environ):
        for prefix in environ_var_prefixes:
            if ev.startswith(prefix):
                print(f"INFO: deleting environment variable {ev}={os.environ[ev]}")
                del os.environ[ev]
    ...
    print("Setting default PATH: {}".format(os.environ["PATH"]))
...
def parseOptions(args=None):
    ...
    global UNIVERSALOPTS, UNIVERSALARCHS, ARCHLIST, CC, CXX  # noqa: PLW0602
    ...
    print(f"   * Third-party source:  {DEPSRC}")
    print(f"   * Deployment target:   {DEPTARGET}")
    print(f"   * Universal archs:     {ARCHLIST!s}")
...
def extractArchive(builddir, archiveName):
    ...
    try:
        ...
        if archiveName.endswith(".tar.gz"):
            ...
            if (retval.startswith(("tcl", "tk"))) and retval.endswith("-src"):
                retval = retval[:-4]
            ...
            fp = os.popen(f"tar zxf {shellQuote(archiveName)} 2>&1", "r")

        elif archiveName.endswith(".tar.bz2"):
            retval = os.path.basename(archiveName[:-8])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen(f"tar jxf {shellQuote(archiveName)} 2>&1", "r")

        elif archiveName.endswith(".tar"):
            retval = os.path.basename(archiveName[:-4])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen(f"tar xf {shellQuote(archiveName)} 2>&1", "r")

        elif archiveName.endswith(".zip"):
            retval = os.path.basename(archiveName[:-4])
            if os.path.exists(retval):
                shutil.rmtree(retval)
            fp = os.popen(f"unzip {shellQuote(archiveName)} 2>&1", "r")

        data = fp.read()
        xit = fp.close()
        if xit is not None:
            sys.stdout.write(data)
            raise RuntimeError(f"Cannot extract {archiveName}")
...
def downloadURL(url, fname):
    ...
    fpOut = open(fname, "wb")  # noqa: SIM115
...
def verifyThirdPartyFile(url, checksum, fname):
    ...
    if os.path.exists(fname):
        ...
        print(f"Did not find local copy of {name}")
        print(f"Downloading {name}")
        downloadURL(url, fname)
        print(f"Archive for {name} stored as {fname}")
    if len(checksum) == 32:
        algo = "md5"
    ...
    if os.system(
        f'CHECKSUM=$(openssl {algo} {shellQuote(fname)}) ; test "${{CHECKSUM##*= }}" = "{checksum}"'
    ):
        fatal(f"{algo} checksum mismatch for file {fname}")
...
def build_universal_openssl(basedir, archList):
    ...
    def build_openssl_arch(archbase, arch):
        ...
        configure_opts = [
            ...
            # "enable-unit-test",
            "shared",
            "--prefix={}".format(os.path.join("/", *FW_VERSION_PREFIX)),
            "--openssldir={}".format(os.path.join("/", *FW_SSL_DIRECTORY)),
        ]
        if no_asm:
        ...
        runCommand("make all")
        runCommand(f"make install_sw DESTDIR={shellQuote(archbase)}")
...
def buildRecipe(recipe, basedir, archList):
    ...
    print(f"Extracting archive for {name}")
    ...
    for patchscript in recipe.get("patchscripts", ()):
        ...
        if fn.endswith(".bz2"):
            runCommand(f"bunzip2 -fk {shellQuote(fn)}")
            fn = fn[:-4]
        runCommand(f"sh {shellQuote(fn)}")
        os.unlink(fn)
    ...
    if configure is not None:
        ...
        if recipe.get("useLDFlags", 1):
            configure_args.extend(
                [
                    ...
                    "LDFLAGS=-mmacosx-version-min={} -L{}/usr/local/lib -arch {}".format(
        ...
        print(f"Running configure for {name}")
    ...
    if install is not None:
        print(f"Running install for {name}")
...
def buildPythonDocs():
    ...
    if doctarfiles:
        ...
        if archivefiles:
            ...
            print(f" -- using pre-built python documentation from {archivefile}")
...
def buildPython():
    print(f"Building a universal python for {UNIVERSALARCHS} architectures")
    ...
    runCommand(
        ...
        "{} "
        "{} "
        "LDFLAGS='-g -L{}/libraries/usr/local/lib' "
        "CFLAGS='-g -I{}/libraries/usr/local/include' 2>&1".format(
            ...
            (
                " ",
                f"--with-openssl='{shellQuote(WORKDIR)[1:-1]}/libraries/usr/local'",
            )[PYTHON_3],
            (" ", "--enable-optimizations --with-lto")[compilerCanOptimize()],
            (
                " ",
                f"TCLTK_CFLAGS='-I{shellQuote(WORKDIR)[1:-1]}/libraries/usr/local/include'",
            )[internalTk()],
            (
                " ",
                f"TCLTK_LIBS='-L{shellQuote(WORKDIR)[1:-1]}/libraries/usr/local/lib -ltcl8.6 -ltk8.6'",
            )[internalTk()],
    ...
    make_cmd = f"make install DESTDIR={shellQuote(rootDir)} {runshared_for_make}"
    print("Running " + make_cmd)
    runCommand(make_cmd)

    make_cmd = f"make frameworkinstallextras DESTDIR={shellQuote(rootDir)} {runshared_for_make}"
    print("Running " + make_cmd)
    runCommand(make_cmd)
    ...
    if os.path.exists(os.path.join(WORKDIR, "libraries", "Library")):
        ...
        if internalTk():
            ...
            runCommand(
                f"mv {shellQuote(build_lib_dir)}/pkgconfig/* {shellQuote(fw_lib_dir)}/pkgconfig"
            )
            runCommand(f"rm -r {shellQuote(build_lib_dir)}/pkgconfig")
        runCommand(f"mv {shellQuote(build_lib_dir)}/* {shellQuote(fw_lib_dir)}")

    frmDir = os.path.join(rootDir, "Library", "Frameworks", "Python.framework")
    frmDirVersioned = os.path.join(frmDir, "Versions", version)
    path_to_lib = os.path.join(frmDirVersioned, "lib", f"python{version}")
    ...
    for dirpath, dirnames, filenames in os.walk(frmDir):
        ...
        for fn in filenames:
            ...
            if fn in EXPECTED_SHARED_LIBS:
                ...
                data = captureCommand(f"otool -L {shellQuote(p)}")
                for sl in EXPECTED_SHARED_LIBS[fn]:
                    if (f"\t{sl} ") not in data:
                        print(f"Expected shared lib {sl} was not linked with {p}")
    ...
    if PYTHON_3:
        ...
        fp = open(os.path.join(buildDir, "Makefile"), "r")  # noqa: SIM115
        for ln in fp:
    ...
    if getVersionMajorMinor() >= (3, 6):
        ...
        path = os.path.join(path_to_lib, f"_sysconfigdata_{ABIFLAGS}_darwin_darwin.py")
    else:
    ...
    if RUNNING_ON_PYTHON2:
        ...
        g_dict = {}
        l_dict = {}
        exec(data, g_dict, l_dict)  # noqa: S102
...
def patchFile(inPath, outPath):
    ...
    data = data.replace("$MACOSX_DEPLOYMENT_TARGET", f"{DEPTARGET} or later")
...
def patchScript(inPath, outPath):
    major, _minor = getVersionMajorMinor()
    data = fileContents(inPath)
...
def packageFromRecipe(targetDir, recipe):
    ...
    try:
        ...
        # common.
        pkgname = "{}-{}".format(recipe["name"], getVersion())
        ...
        fp = open(fn, "w")  # noqa: SIM115
        fp.write("pmkrpkg1")
        fp.close()

        rsrcDir = os.path.join(packageContents, "Resources")
        os.mkdir(rsrcDir)
        fp = open(os.path.join(rsrcDir, "ReadMe.txt"), "w")  # noqa: SIM115
        fp.write(readme)
        fp.close()
        ...
        pl = {
            "CFBundleGetInfoString": f"Python.{pkgname} {vers}",
            "CFBundleIdentifier": f"org.python.Python.{pkgname}",
            "CFBundleName": f"Python.{pkgname}",
        ...
        pl = {
            ...
            "IFPkgDescriptionTitle": recipe.get("long_name", f"Python.{pkgname}"),
...
def makeMpkgPlist(path):
    ...
    pl = {
        "CFBundleGetInfoString": f"Python {vers}",
...
def installSize(clear=False, _saved=None):
    if _saved is None:
        _saved = []
    if clear:
        del _saved[:]
...
def buildDMG():
    ...
    if build_tuple[0] < 11:
        os_name = "macosx"
        build_system_version = "{}.{}".format(*build_tuple)
    else:
        os_name = "macos"
    ...
    volname = f"Python {getFullVersion()}"
    cmd = "hdiutil create -format UDRW -volname {} -srcfolder {} -size 100m {}".format(
        shellQuote(volname),
    ...
    runCommand(
        "hdiutil attach {} -mountroot {}".format(
...
def main():
    ...
    folder = os.path.join(WORKDIR, "_root", "Applications", f"Python {getVersion()}")
    ...
    fp.write(f"# By: {pwd.getpwuid(os.getuid()).pw_gecos}\n")
    fp.close()
